...
<body>
	...
	<script>
			...
			if ($(".myChartArea").css("display") == "none") {
				...
				$.ajax({
					...
					success : function(res){
						// drawChart함수에서 쓸 label 데이터, data 데이터 만들기
						var artistList = [];
						var cntList = [];
						for(var i =0; i<res.length; i++){
							artistList.push(res[i].artist);
							cntList.push(res[i].keywordCnt);
						}
						
						// drawChart 함수 실행
						// 차트 그리기
						drawChart(artistList, cntList);
		...
		function drawChart(artistList, cntList) {
			// 차트 그려주는 영역
			...
			const data = {
				...
				datasets : [ {
					...
					backgroundColor : [ 'rgb(255, 99, 132)',
							'rgb(54, 162, 235)', '000000',
							'#fdf0d5','rgb(100, 180, 95)'],
					hoverOffset : 4
				} ]
			};
